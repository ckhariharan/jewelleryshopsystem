...
export class Ser1Service {
  ...
  updateDocument(requests: Array<any>) {
    this.httpClient.post(this.couchDBUrl + '/' + this.databaseName + '/_bulk_docs', { 'docs': requests }, {
      ...
      if (res[0]['ok']) {
        Swal.fire('Buy successful', 'Thanks for Buying Our Products', 'success');
      }
  ...
  async searchDocument(query: string) {
    return this.httpClient.post(this.couchDBUrl + '/' + this.databaseName + '/_design/jwsilver/_search/creditor_search',
